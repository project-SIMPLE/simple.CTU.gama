	...
	output {
		...
		/////P1
			...
			graphics "image4" refresh: false size: player_size position: player_4_position {
				...
				draw "Sụt lún: " + (mean(cell collect each.subsidence(GPlayLand[nP].playerLand_ID)) with_precision 2) at: {3e5, 58000} font: font("Helvetica", 24, #bold) color: #yellow;
				draw "Time: " + current_date.hour + ":" + current_date.minute at: {3e5, 78000} font: font("Helvetica", 24, #bold) color: #yellow;
				draw "P4 " + (length(unity_player) > 3 ? (unity_player[nP].name) : "") at: {3e5, 98000} font: font("Helvetica", 24, #bold) color: #yellow;
			}
