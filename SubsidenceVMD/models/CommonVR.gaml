...
//name of the species used to represent a Unity player
	...
	reflex send_enemy_spawn_rate when: every(enemy_genetation_rate_refresh_rate #cycle) {
		ask unity_player where not (dead(each) and each.ready_to_start) {
			...
			//					refill_rates[int(myland)]<-refill_rates[int(myland)]*length(myland.downstream.pumpers)/10;
			////					spawn_rate<-spawn_rate-1;
			//				}
				if (myland.upstream != nil) {
					refill_rates[int(myland)] <- 0.0001 + length(myland.upstream.pumpers) / 10000;
					//					spawn_rate<-spawn_rate+1;
				}
	...
	reflex send_info_subsidence when: every(update_subsidence_refresh_rate #cycle) {
		...
		ask unity_player where not (dead(each) and each.ready_to_start) {
			...
			ask myself {
				do
				send_message([myself], ["subsidences"::subsidence_values, "subsi_score"::subsi_score, "waterGlobal"::round(precision * sum(waters) / (length(GPlayLand) * sum_water_init)), "waterLocal"::round(precision * 100 * waters[myself.myland.playerLand_ID] / sum_water_init)]);
			}
	...
	action move_create_pumper (string idP, string idwp, int x, int y) {
		...
		if (wp != nil) {
			...
			create Pumper {
				...
				Pl.myland.numberPumper <- Pl.myland.numberPumper + 1;
				location <- pt;
