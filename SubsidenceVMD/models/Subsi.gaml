...
//		loop cell_temp over: flooding   {
//			if(cell_temp!=nil){
////			write flooding[geometry(cell_temp).location] ;
////			write allField[geometry(cell_temp).location];
////			write  DEM[geometry(cell_temp).location];
//			if (DEM[geometry(cell_temp).location]> -9999) and (allField[geometry(cell_temp).location]> -9999){ 
//				flooding[geometry(cell_temp).location]  <- DEM[geometry(cell_temp).location] - allField[geometry(cell_temp).location] - 0.015;	
//			}			
//			
//			}
//		}
...
experiment main type: gui {
	output {
		...
		display "Flooding-SLR 15cm" type: 3d {			
			species boundMK;
			mesh cell_dem color:scale([#darkblue::-10,#blue::-3, #lightblue::-1, #cyan::-0.001, #red::1, #darkred::2,#black::30 ]) no_data:-9999 smooth: false;
		}
